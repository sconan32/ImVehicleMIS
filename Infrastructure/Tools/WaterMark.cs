...
namespace Socona.ImVehicle.Infrastructure.Tools
{
    ...
    public class WaterMark
    {
        ...
        int[] sizes = new int[] { 14, 14, 14, 8, 6, 4 };
        ...
        #region ---------------------方法事件---------------------
        public void Mark(MemoryStream imgStream, MarkType markType, string text, Image waterImg, int markx, int marky,
            bool bold, Color textColor, float transparence, FontFamily fontFamily)
        {
            ...
            try
            {
                ...
                if (markType == MarkType.Text)
                {
                    ...
                    //创建刷子对象，准备给图片写上文字
                    Brush brush = new SolidBrush(Color.FromArgb(30,10,10,10));
                    //写阴影

                    g.DrawString(text, cFont, brush, markx + 4, marky + 4);
                    g.DrawString(text, cFont, brush, markx - 1, marky - 1);
                    g.DrawString(text, cFont, brush, markx - 1, marky + 1);
                    g.DrawString(text, cFont, brush, markx + 1, marky - 1);
                    brush = new SolidBrush(Color.FromArgb(70, 10, 10, 10));
                    g.DrawString(text, cFont, brush, markx + 3, marky + 3);
                    brush = new SolidBrush(Color.FromArgb(110, 10, 10, 10));
                    g.DrawString(text, cFont, brush, markx + 2, marky + 2);

                    brush = new SolidBrush(Color.FromArgb(140, 10, 10, 10));
                    //写阴影
                    ...
                    newBitmap.Save(imgStream, img.RawFormat);
                    imgStream.Close();
                ...
                else if (markType == MarkType.Image)
                {
                    ...
                    {
                        System.DrawingCore.Image.GetThumbnailImageAbort callb = null;
                        //对水印图片生成缩略图,缩小到原图得1/4
                        System.DrawingCore.Image new_img = markImg.GetThumbnailImage(img.Width / 4, markImg.Height * img.Width / markImg.Width, callb, new System.IntPtr());
                        //添加水印
